...
class PayIrGatewayTest(unittest.TestCase):
    _config = {"pin": "8523528", "callback_url": "http://localhost/callback"}
    ...
    def test_pay_ir(self, _, __):
        ...
        invalid_transaction = gateway.validate_transaction(
            {"transId": "RETURNED_TOKEN", "status": "300"}
        )
        ...
        @mock.patch(
            ...
            side_effect=get_side_effect(
                returned_status=1, returned_amount=1000
            ),
        )
        ...
        @mock.patch(
            self._mock_path,
            side_effect=get_side_effect(
                returned_status=300, returned_amount=400
            ),
        )
        ...
        @mock.patch(
            self._mock_path,
            side_effect=get_side_effect(
                returned_status=1, returned_amount=600
            ),
        )
        ...
        @mock.patch(
            self._mock_path, side_effect=get_side_effect(raise_http_error=402)
        )
        ...
        @mock.patch(
            self._mock_path, side_effect=get_side_effect(raise_http_error=422)
        )
        ...
        @mock.patch(
            self._mock_path, side_effect=get_side_effect(raise_url_error=True)
        )
