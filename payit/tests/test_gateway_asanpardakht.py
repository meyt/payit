...
class AsanPardakhtGatewayTest(unittest.TestCase):
    ...
    @mock.patch(
        "payit.gateways.asanpardakht.Client", side_effect=get_side_effect()
    )
    def test_gateway(self, _):
        ...
        valid_transaction = gateway.validate_transaction(
            self._generate_callback_data(
                amount=1000, sale_order_id=1, res_code=0
            )
        )
        ...
        invalid_transaction = gateway.validate_transaction(
            self._generate_callback_data(
                amount=1000, sale_order_id=1, res_code=300
            )
        )
        self.assertEqual(invalid_transaction.validate_status, False)

        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(returned_token="300"),
        )
        ...
        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(verify_result=500),
        )
        def test_verify_success(_):
            gateway.verify_transaction(
                transaction, self._generate_callback_data()
            )
        ...
        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(verify_result=100),
        )
        ...
        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(reconcile_result=300),
        )
        ...
        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(verify_result=505),
        )
        ...
        @mock.patch(
            "payit.gateways.asanpardakht.Client",
            side_effect=get_side_effect(verify_result=-900),
        )
