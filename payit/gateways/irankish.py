...
class IrankishGateway(Gateway):
    ...
    def verify_transaction(self, transaction: Transaction, data):
        try:
            params = {
                'merchantId': self.config['merchant'],
                'referenceNumber': data['referenceId'] if 'referenceId' in data else 0,
            ...
            if result > 0:
                ...
                raise TransactionAlreadyPaidError('Irankish: transaction already verified')
            else:
                raise TransactionError('Irankish: invalid transaction, %s' % result)
        except zeep_exceptions.Error:
            raise GatewayNetworkError
